...
import { deleteReportForm, fetchReportForms } from 'src/features/store/apps/tools/reportForms'
...
const ReportFormsListAdmin = () => {
  ...
  const handleShowDialog = async (arr?: string[]) => {
    ...
    if (confirmed) {
      if (arr) {
        handleDeleteReportForm(arr)
      } else {
        handleDeleteReportForm(Object.keys(rowSelection))
        setRowSelection({})
      }
    } else {
      setRowSelection({})
  ...
  const handleDeleteReportForm = async (arr: string[]) => {
    const toastId = toast.loading(t('ApiLoading'))
    await dispatch(deleteReportForm(arr))
      .unwrap()
      .then(() => {
  ...
  const columns = useMemo<MRT_ColumnDef<ReportFormType>[]>(
    () => [
      ...
      {
        accessorKey: 'items_filled_count',
        accessorFn: row => row.items_filled_count,
        id: 'items_filled_count',
        header: t('FilledCount'),
        Cell: ({ row }) => (
          <Typography>
            {row.original.items_filled_count !== null
              ? row.original.items_filled_count + ' / ' + row.original.items_count
              : ''}
          </Typography>
        )
      }
  ...
  const table = useMaterialReactTable({
    ...
    renderRowActions: ({ row }) => (
      <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-around' }}>
        ...
        {ability?.can('write', 'tool_report_forms') ? (
          <Button
            variant='tonal'
            color='error'
            size='small'
              ...
              <Grid container spacing={6}>
                ...
                <Grid item xs={12} sm={12} md={3} lg={3}>
                  <Button
                    ...
                    variant='contained'
                    fullWidth
                    href='/tools/report-forms/create'
                    startIcon={<Icon icon='tabler:plus' />}
                  >
                    <Translations text='AddReportForm' />
                  </Button>
                </Grid>
              </Grid>
            </CardContent>
...
export default ReportFormsListAdmin

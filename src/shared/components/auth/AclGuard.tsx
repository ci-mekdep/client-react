...
// ** Types
import type { ACLObj, AppAbility } from 'src/app/configs/acl'

// ** Context Imports
import { AbilityContext } from 'src/app/layouts/components/acl/Can'

// ** Config Import
import { buildAbilityFor } from 'src/app/configs/acl'
...
const AclGuard = (props: AclGuardProps) => {
  ...
  }, [auth.current_role, auth.user, guestGuard, router])

  // User is logged in, build ability for the user based on his role
  if (auth.user && !ability) {
    ability = buildAbilityFor(
      auth.current_role as string,
      auth.user.permissions as string[],
      auth.user.permissions_write as string[]
    )
    if (router.route === '/') {
      return <Spinner />
    }

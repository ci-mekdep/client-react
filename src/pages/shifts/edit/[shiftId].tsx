// ** React Imports
import { FormEvent, ReactElement, Ref, SyntheticEvent, forwardRef, useEffect, useState } from 'react'
...
// ** Store Imports
import { getCurrentShift, updateShift } from 'src/features/store/apps/shifts'
import { fetchSchoolsLite } from 'src/features/store/apps/school'

// ** Utils Imports
import generateShift from 'src/features/utils/generateShift'
import { convertShiftData } from 'src/features/utils/api/convertShiftData'
...
const findEnabledDays = (shift: (string[] | null)[][]) => {
  const enabledDaysSet = new Set()
  shift.forEach(period => {
    period.forEach((day, dayIndex) => {
      if (day && day.length > 0) {
        enabledDaysSet.add(dayIndex)
      }
    })
  })

  return Array.from(enabledDaysSet)
}
...
const ShiftEdit = () => {
  ...
  const [shiftTimes, setShiftTimes] = useState<(string[] | null)[][]>(defaultShiftTimes)
  ...
  const currentShiftId = router.query.shiftId
  const { is_secondary_school } = useAuth()
  const dispatch = useDispatch<AppDispatch>()
  const { shift_detail, shift_update } = useSelector((state: RootState) => state.shifts)
  ...
  useEffect(() => {
    if (currentShiftId !== undefined) {
      dispatch(getCurrentShift(currentShiftId as string))
    }
  ...
  useEffect(() => {
    if (
      ...
      !shift_detail.loading &&
      shift_detail.status === 'success' &&
      schools_lite_list.data.length !== 0
    ) {
      setSchool(
        schools_lite_list.data.find((school: LiteModelType) => school.key === shift_detail.data?.school?.id) || null
      )
      setFormData(convertShiftData(shift_detail.data))
      setShiftTimes(reverseArray(shift_detail.data?.value))

      const newEnabledDays = findEnabledDays(reverseArray(shift_detail.data?.value))
      setEnabledDays(newEnabledDays as number[])
    }
  }, [currentShiftId, shift_detail, schools_lite_list.data])
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement> | null, data: ShiftCreateType, is_list: boolean) => {
  ...
  if (!shift_detail.loading && shift_detail.status === 'success') {
    return (
      <>
        <Dialog
          fullWidth
              ...
              <Grid container spacing={6}>
                ...
                <Grid item xs={12} sm={6}>
                  <CustomTextField
                    ...
                    onChange={e =>
                      setGenerateShiftData({ ...generateShiftData, lessonDuration: Number(e.target.value) })
                    }
                    InputProps={{
                      endAdornment: (
                ...
                <Grid item xs={12} sm={4}>
                  <CustomTextField
                    ...
                    onChange={e =>
                      setGenerateShiftData({ ...generateShiftData, bigBreakIndex: Number(e.target.value) })
                    }
                    InputProps={{
                      startAdornment: (
                        <InputAdornment position='start'>
          ...
          >
            <Grid container spacing={6}>
              ...
              <Grid item xs={12} sx={{ textAlign: 'right', pt: theme => `${theme.spacing(6.5)} !important` }}>
                <Button variant='contained' sx={{ mr: 4 }} disabled={shift_update.loading} type='submit'>
                  {shift_update.loading ? (
                    <CircularProgress
                      sx={{

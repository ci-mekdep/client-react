// ** React Imports
import { useState, useEffect, SyntheticEvent, FormEvent, ChangeEvent } from 'react'
...
import { convertBaseSubjectData } from 'src/features/utils/api/convertBaseSubjectData'
import { getCurrentBaseSubject, updateBaseSubject } from 'src/features/store/apps/baseSubjects'
...
const BaseSubjectEdit = () => {
  ...
  const currentBaseSubjectId = router.query.baseSubjectId
  const { base_subject_detail, base_subject_update } = useSelector((state: RootState) => state.baseSubjects)
  const { settings } = useSelector((state: RootState) => state.settings)
  const { schools_lite_list } = useSelector((state: RootState) => state.schools)

  useEffect(() => {
    if (currentBaseSubjectId !== undefined) {
      dispatch(getCurrentBaseSubject(currentBaseSubjectId as string))
    }
  }, [dispatch, currentBaseSubjectId])
  ...
  useEffect(() => {
    if (
      currentBaseSubjectId !== undefined &&
      !base_subject_detail.loading &&
      base_subject_detail.status === 'success' &&
      schools_lite_list.data.length !== 0 &&
      !settings.loading &&
      settings.data &&
      settings.data.subject.base_subjects.length !== 0
    ) {
      const detailData: BaseSubjectType = { ...(base_subject_detail.data as BaseSubjectType) }
      setFormData(convertBaseSubjectData(detailData))
      ...
      if (detailData.is_available !== null) {
        setIsAvailable(detailData.is_available)
      }
    }
  }, [currentBaseSubjectId, schools_lite_list.data, base_subject_detail, settings])
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement> | null, data: BaseSubjectCreateType, is_list: boolean) => {
    event?.preventDefault()

    const obj = { ...data, price: data.price ? parseInt(data.price.toString() + '00', 10) : null }

    dispatch(updateBaseSubject(obj))
      .unwrap()
      .then(res => {
  ...
  if (!base_subject_detail.loading && currentBaseSubjectId) {
    return (
      <>
        <form
          autoComplete='off'
        ...
        >
          <Grid container spacing={6}>
            ...
            <Grid item xs={12} sx={{ textAlign: 'right', pt: theme => `${theme.spacing(6.5)} !important` }}>
              <Button variant='contained' sx={{ mr: 4 }} disabled={base_subject_update.loading} type='submit'>
                {base_subject_update.loading ? (
                  <CircularProgress
                    sx={{
              ...
              <Button
                variant='contained'
                sx={{ mr: 4 }}
                disabled={base_subject_update.loading}
                onClick={() => {
...
export default BaseSubjectEdit
